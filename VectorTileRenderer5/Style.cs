...
namespace VectorTileRenderer
{
    ...
    public class Paint
    {
        ...
        public SKPoint FillTranslate { get; set; } = new Point();
    ...
    public class Style
    {
        ...
        private SKColor parseColor(object iColor)
        {
            ...
            if (colorString.StartsWith("hsla("))
            {
                ...
                float s = float.Parse(segments[2]);
                float l = float.Parse(segments[3]);
                byte a = (byte)(float.Parse(segments[4]) * 255);
            ...
            {
                var segments = colorString.Replace('%', '\0').Split(',', '(', ')');
                byte r = (byte)(double.Parse(segments[1]) * 255);
                byte g = (byte)(double.Parse(segments[2]) * 255);
                byte b = (byte)(double.Parse(segments[3]) * 255);
                byte a = (byte)(double.Parse(segments[4]) * 255);
