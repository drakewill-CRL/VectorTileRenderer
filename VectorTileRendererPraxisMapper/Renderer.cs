...
namespace VectorTileRendererPraxisMapper
{
    public class Renderer
    {
        ...
        public async static Task<SkiaSharp.SKBitmap> RenderCached(string cachePath, Style style, SkiaCanvas canvas, int x, int y, double zoom, double sizeX = 512, double sizeY = 512, double scale = 1)
        {
            var bundle = new
            {
                style.Hash,
            ...
            return SkiaSharp.SKBitmap.Decode(bitmap);
        }

        static SkiaSharp.SKBitmap loadBitmap(string path)
        {
            using (var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                var fsBitmap = SkiaSharp.SKBitmap.Decode(stream);
        ...
        public async static Task<byte[]> Render(Style style, SkiaCanvas canvas, int x, int y, double zoom, double sizeX = 512, double sizeY = 512, double scale = 1)
        {
