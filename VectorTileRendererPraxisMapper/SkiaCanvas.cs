...
namespace VectorTileRendererPraxisMapper
{
    ...
    //Notes: call StartDrawing, then call for each element, then call FinishDrawing to get a bitmap
    public class SkiaCanvas //: ICanvas
    {
        ...
        public void StartDrawing(double width, double height)
        {
            ...
            bitmap = new SKBitmap(this.width, this.height,  SKColorType.Rgba8888, SKAlphaType.Premul);
            //bitmap.Lock();
        ...
        public void DrawBackground(SKPaint style)
        {
        ...
        {
            return cap; //Not needed if I don't convert.
        }
        ...
        public void DrawLineString(List<Point> geometry, Brush style)
        {
            ...
            SKPaint fillPaint = new SKPaint
            {
                ...
                StrokeCap = style.Paint.LineCap,
                StrokeWidth = (float)style.Paint.LineWidth,
                Color = style.Paint.LineColor,
                IsAntialias = true,
            };
        ...
        SKTextAlign convertAlignment(SKTextAlign alignment) //NO longer actually converting this.
        {
            return alignment;
            //return alignment;
        ...
        SKPaint getTextStrokePaint(Brush style)
        {
            var paint = new SKPaint()
            {
                ...
                Color = style.Paint.TextStrokeColor, //new SKColor(style.Paint.TextStrokeColor.R, style.Paint.TextStrokeColor.G, style.Paint.TextStrokeColor.B, (byte)clamp(style.Paint.TextStrokeColor.A * style.Paint.TextOpacity, 0, 255)),
        ...
        SKPaint getTextPaint(Brush style)
        {
            var paint = new SKPaint()
            {
                Color = style.Paint.TextColor, //new SKColor(style.Paint.TextColor.R, style.Paint.TextColor.G, style.Paint.TextColor.B, (byte)clamp(style.Paint.TextColor.A * style.Paint.TextOpacity, 0, 255)),
                TextSize = (float)style.Paint.TextSize,
                IsAntialias = true,
                TextEncoding = SKTextEncoding.Utf32,
                TextAlign = convertAlignment(style.Paint.TextJustify),
